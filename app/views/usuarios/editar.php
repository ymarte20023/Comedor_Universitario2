                ...
                <form action="<?php echo URLROOT; ?>/usuarios/editar/<?php echo $data['usuario']['id']; ?>" method="POST">
                    <div class="form-group">
                        ...
                        <input type="text" id="nombre" name="nombre" required 
                               ...
                               value="<?php echo htmlspecialchars($data['usuario']['nombre']); ?>">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="email" name="email" required 
                               ...
                               value="<?php echo htmlspecialchars($data['usuario']['email']); ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" id="password" name="password" 
                               ...
                               placeholder="••••••••">
                        <small class="text-muted">Complejidad: 1 mayúscula, 1 número y 1 símbolo.</small>
                    </div>

                    <div class="form-group">
                        <label for="rol">Rol de Usuario</label>
                        <?php if ($data['usuario']['rol'] === 'administrador'): ?>
                            <input type="text" value="Administrador" disabled style="background: #f5f5f5;">
                            <input type="hidden" name="rol" value="administrador">
                            <small class="text-muted">El rol de administrador no se puede cambiar desde aquí.</small>
                        <?php else: ?>
                            <select id="rol" name="rol" required>
                                <option value="inventario" <?php echo $data['usuario']['rol'] == 'inventario' ? 'selected' : ''; ?>>Inventario</option>
                                <option value="cocina" <?php echo $data['usuario']['rol'] == 'cocina' ? 'selected' : ''; ?>>Cocina</option>
                            </select>
                        <?php endif; ?>
                    </div>

                    <div style="margin-top: 2rem;">
                        <button type="submit" class="btn btn-primary btn-lg" style="width: 100%;">Actualizar Usuario</button>
                    </div>
                </form>
            </div>
        </main>
